...
def prepare_model(checkpoint, input_dir):

    make_deterministic()
    device = utils.get_device()
    logger = utils.logger_setup(logpath=os.path.join(input_dir, f'logs_{time.time()}'), filepath=os.path.abspath(__file__))
    loaded_args, model, _ = utils.load_model(checkpoint, logger, device, model_mode=ModelModes.EVALUATION,
...
def compress_and_decompress(args):
    ...
    loaded_args, model, _ = utils.load_model(args.checkpoint, logger, device, model_mode=ModelModes.EVALUATION,
    ...
    bpp_total, q_bpp_total, SSIM_total, LPIPS_total = torch.Tensor(N), torch.Tensor(N), torch.Tensor(N), torch.Tensor(N)
    ...
    with torch.no_grad():
        ...
        for idx, (data, bpp, filenames) in enumerate(tqdm(eval_loader), 0):
            ...
            data = F.upsample(data, size=(data.size(2)//2, data.size(3)//2), mode='bicubic')
            perceptual_loss = model.perceptual_ssim_loss(reconstruction, data)
            ...
            SSIM_total[n:n + B] = perceptual_loss.data
            n += B
    ...
    df['SSIM'] = 1 - SSIM_total.cpu().numpy()
    ...
    pprint(np.mean(df["MS_SSIM"]))
    pprint(np.mean(df["SSIM"]))
    # pprint(np.mean(df["LPIPS"]))
    pprint(np.mean(df["PSNR"]))
    pprint(np.mean(df["bpp_original"]))
    pprint(np.mean(df["q_bpp"]))
